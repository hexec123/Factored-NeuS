...
def run_colmap_reconstruction(images_dir: Path, work_dir: Path, colmap_bin: str,
                              matcher: str, camera_model: str, masks_dir: Optional[Path]=None) -> Tuple[Path, Path]:
    ...
    feat_cmd = [colmap_bin, "feature_extractor",
                "--database_path", str(db_path),
                "--image_path", str(images_dir),
                "--ImageReader.camera_model", camera_model,
                "--ImageReader.single_camera", "1"]
    if masks_dir is not None:
        feat_cmd += ["--ImageReader.mask_path", str(masks_dir)]
    run(feat_cmd)
...
def copy_and_rename_images_with_masks(src_dir: Path, dst_dir: Path, masks_src_dir: Optional[Path], masks_dst_dir: Path):
    if dst_dir.exists(): shutil.rmtree(dst_dir)
    dst_dir.mkdir(parents=True)
    if masks_dst_dir.exists(): shutil.rmtree(masks_dst_dir)
    masks_dst_dir.mkdir(parents=True)
    for i,p in enumerate(sorted(src_dir.iterdir())):
        if p.suffix.lower() not in [".jpg",".jpeg",".png"]: continue
        img = cv2.imread(str(p))
        cv2.imwrite(str(dst_dir/f"{i:06d}.png"), img)
        # find corresponding mask by original filename in masks_src_dir
        if masks_src_dir is not None:
            src_mask = masks_src_dir / p.name
            if not src_mask.exists():
                # try with .png extension
                src_mask = masks_src_dir / (p.stem + '.png')
            if src_mask.exists():
            ...
            h,w = img.shape[:2]; m = np.ones((h,w),np.uint8)*255
        cv2.imwrite(str(masks_dst_dir/f"{i:06d}.png"), m)


def save_masks(images_dir: Path, masks_dir: Path):
    # legacy: ensure a mask directory of same-sized white masks exists
...
def main():
    ...
    # Detect existing masks (in public_data/[scene]/mask) and run COLMAP using them if present
    existing_masks_dir = scene_dir/'mask'
    masks_for_colmap: Optional[Path] = None
    if existing_masks_dir.exists() and any(p.suffix.lower() in ['.png', '.jpg', '.jpeg'] for p in existing_masks_dir.iterdir()):
        masks_for_colmap = existing_masks_dir

    # Run COLMAP + export TXT + PLY (pass mask dir if available)
    txt_dir, default_ply = run_colmap_reconstruction(images_tmp, work_dir, args.colmap_bin, args.colmap_matcher, args.camera_model, masks_dir=masks_for_colmap)
    ...
    # Copy images & masks: preserve user-provided masks if present, otherwise generate white masks
    dst_image_dir = scene_dir/'image'
    dst_mask_dir = scene_dir/'mask'
    try:
        same_dir = images_tmp.resolve() == dst_image_dir.resolve()
    except Exception:
        same_dir = False

    if same_dir:
        # Source and destination are identical — don't delete images. Ensure masks are present.
        if masks_for_colmap is not None:
            # Align mask names to image names. Work in a temp dir if source==dest to avoid overwriting.
            masks_list = list(masks_for_colmap.iterdir())
            need_temp = masks_for_colmap.resolve() == dst_mask_dir.resolve()
            temp_dir = dst_mask_dir.parent / (dst_mask_dir.name + "_tmp") if need_temp else dst_mask_dir
            if temp_dir.exists(): shutil.rmtree(temp_dir)
            temp_dir.mkdir(parents=True, exist_ok=True)

            for img_p in sorted(dst_image_dir.iterdir()):
                if img_p.suffix.lower() not in [".jpg",".jpeg",".png"]: continue
                img_name = img_p.name
                # try exact match
                candidate = masks_for_colmap / img_name
                found = None
                if candidate.exists():
                    found = candidate
                else:
                    # try suffix match (e.g., mask '000000.png' endswith '000.png')
                    for m in masks_list:
                        if m.is_file() and m.name.endswith(img_name):
                            found = m
                            break
                if found is not None:
                    shutil.copyfile(str(found), str(temp_dir / img_name))
                else:
                    # generate white mask
                    img = cv2.imread(str(img_p)); h,w = img.shape[:2]
                    m = np.ones((h,w),np.uint8)*255
                    cv2.imwrite(str(temp_dir / img_name), m)

            if need_temp:
                # replace original mask dir
                if dst_mask_dir.exists(): shutil.rmtree(dst_mask_dir)
                temp_dir.rename(dst_mask_dir)
        else:
            # ensure there is at least a white mask per image
            save_masks(dst_image_dir, dst_mask_dir)
    else:
        copy_and_rename_images_with_masks(images_tmp, dst_image_dir, masks_for_colmap, dst_mask_dir)
